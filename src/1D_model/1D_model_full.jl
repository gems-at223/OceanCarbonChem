...
CO₂_sink_rate = 10e-9/3600 # Example sink rate value

function CO₂_sink(z,t,c₁)
    return -(CO₂_sink_rate/(4*pi*(z)^2))*(c₁/((5*10^-6)+c₁))
end

function HCO₃_sink(z,t,c₁)
    return -(CO₂_sink_rate/(4*pi*(z)^2))*(1-(c₁/((5*10^-6)+c₁)))
end
...
function total_HCO₃_uptake( z,t,c₁)
    total_uptake = 0.0
    for z in grid.zC
        total_uptake += HCO₃_sink(z, t, c₁) * grid.Δz
    end
    return total_uptake
end
...
function OH_forcing_func(z,t, c₁)
    if z==0.000
        return -total_HCO₃_uptake(z,t, c₁)
    else
        return 0
    end
end
...
CO₃_flux = 3e-9/3600
boundary_conditions = (c₃ = FieldBoundaryConditions(top = FluxBoundaryCondition(CO₃_flux), bottom = GradientBoundaryCondition(0.0)),
...
model = NonhydrostaticModel(; grid,
                              biogeochemistry,
                              closure = ScalarDiffusivity(κ=(c₁ = 2e-9, c₂ = 1.18e-9, c₃ = 0.955e-9,c₄=9.18e-9, c₅ = 5.27e-9, c₆ = 1.11e-9, c₇ = 0.98e-9)),
...
simulation = Simulation(model, Δt = 0.0000002seconds, stop_time = 0.01seconds)

simulation.output_writers[:tracers] = JLD2OutputWriter(model, model.tracers,
                                                       filename = "column_np.jld2",
                                                       schedule = TimeInterval(0.003seconds),
...
axHCO3 = Axis(fig[1, 1], ylabel = "z (m)")
...
BOH3_line=Axis(fig_line[3, 2], xlabel = "Length (m)", ylabel = "B(OH)₃ (mol / kg)")
BOH4_line=Axis(fig_line[2, 2], xlabel = "Length (m)", ylabel = "B(OH)₄⁻ (mol / kg)")
...
lines!(BOH3_line, zc*-1,BOH3_profile, label = "BOH3 at final time step")
lines!(BOH4_line, zc*-1,BOH4_profile, label = "BOH4 at final time step")
