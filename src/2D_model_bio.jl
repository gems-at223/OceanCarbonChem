...
boundary_conditions = (c₃ = FieldBoundaryConditions(top = GradientBoundaryCondition(0.0), bottom = GradientBoundaryCondition(0.0)),
                       c₂ = FieldBoundaryConditions(top = FluxBoundaryCondition(CO₂_flux), bottom = FluxBoundaryCondition(CO₂_flux),
                       west = FluxBoundaryCondition(CO₂_flux), east = FluxBoundaryCondition(CO₂_flux)),
...
simulation.output_writers[:tracers] = JLD2OutputWriter(model, model.tracers,
                                                       ...
                                                       schedule = TimeInterval(0.001*20seconds),
...
Colorbar(fig[3, 2], hm3, ticks = [ 0.0 , 0.000315])
