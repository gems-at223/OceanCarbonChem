/* -------------------------------------------------------------
   ...
   updates:
     ...
     XX/05/20 updates to run from command line and interace with python
...
#define CMD    /* FORAMW,FORAMOUD,FORAMOUL,FORAMGSD,FORAMGSL
			           FORAMRES,FORAMPHS,FORAMCLC
			           FORAMBORD FORAMBORL
                    PYTHON  -  parameters set by python                        
                    CMD - run from command line specifying arguments */
...
#ifdef CMD                /* Parameters defined by Python */
#define RADIUS 2.500000000e+02      /* radius of foram [mu] diatoms */
  /*  UPTAKE at the shell :           */
#define CO3UPT   0.000000000e+00  /* .75/3.25 direct calcification */
#define CO2UPT  0.000000000e+00 /* 3+2[mol CO2 / s / foram] respiration */
#define TCO2UPT CO2UPT
#define HCO3UPT 0.000000000e+00
#define O2UPT (-CO2UPT)
#define CAUPT   CO3UPT
#define OHUPT 0.0
#define HUPT  (0.0e-20)
...
#define PHBULK  8.063000000e+00    /* 8.2 /8.16      */
#define DICBULK 4.035000000e+03       /* 2167.      */
#define UALKBULK 4.671000000e+03    /* 2723.      */

/*  UPTAKE of symbionts : 16.2 = 18 (gross) - 1.8 (symb. resp.)*/
#define SYMBIONTS/*----     1. set CO2 and HCO3- uptake     --------*/
#define SYMCO2UPT 0.000000000e+00  /* CO2  uptake by symbionts */
#define SYMHCO3UPT  0.000000000e+00  /* HCO3 uptake by symbionts */
#define SYMHUPT SYMHCO3UPT    /* H    uptake by symbionts */

#define MIMECO2SYM/*----   2. MIME: set total carbon  uptake     ---*/
#define SYMTCUPT 2.777777778e-12
#define VMAX 3.333333333e-12    /* Michaelis-Menten max upt. for CO2 */
#define KS 5.         /* [mumol/l] Michaelis-Menten half sat. */
#define DVDIT 3.0   /* increase of uptake per iteration */
#define SYMRAD (RADIUS+5.000000000e+02)   /* 500/200 outer radius for symbiont distribution */
#define SYMO2UPT (-(SYMCO2UPT+SYMHCO3UPT))
#define REDS 1.000000000e+00    /* Redfield ratio symbiont photosynth.  */
...
/* Added 25/01/2017 by Branson and Holland! */
#if defined CMD
#define SALINITY 3.330000000e+01
#define TEMP 2.200000000e+01
#endif

/* Added 25/01/2017 by Branson and Holland! */
#ifdef CMD
#define BORTBULK 2.055085714e+03 /* [mumol/kg], DOE94  */
#else
...
#define SLOWC 0.3    /* fraction of correction def. = 1.0 */
#define ITMAX 100     /* def. = 30 max. number of iterations */
...
#endif
{
...
#ifdef CMD
   // double rad = atol(argv[1]);
   // printf("input is %d\n\n", rad);
   // double CO3UPT = strtod(argv[2], NULL);
   // double CO2UPT = strtod(argv[3], NULL);
   // #define TCO2UPT CO2UPT

   // double HCO3UPT = strtod(argv[4], NULL);
   // #define O2UPT (-CO2UPT)
   // #define CAUPT   CO3UPT
   // #define OHUPT 0.0
   // #define HUPT  (0.0e-20)
   ...
   // double PHBULK = strtod(argv[1], NULL);
   // double DICBULK = strtod(argv[6], NULL);
   
   // #define SYMBIONTS
   // double SYMCO2UPT = strtod(argv[7], NULL);
   // double SYMHCO3UPT = strtod(argv[8], NULL);
   // #define SYMHUPT SYMHCO3UPT

   // #define MIMECO2SYM
   // double SYMTCUPT = strtod(argv[9], NULL);
#endif
